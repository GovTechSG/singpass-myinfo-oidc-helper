...
/* harmony import */ var _util_RedactorUtil__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../util/RedactorUtil */ "./src/util/RedactorUtil.ts");
...
const getProxyConfig = () => {
    ...
    if (proxyUrl) {
        const parsed = url__WEBPACK_IMPORTED_MODULE_2__["parse"](proxyUrl);
        const proxyConfig = {
        ...
        if (!!parsed.auth) {
            ...
            Object(lodash__WEBPACK_IMPORTED_MODULE_1__["set"])(proxyConfig, "auth", {
                username,
                password,
            });
        }
...
const addRequestLogs = (instance) => {
    instance.interceptors.request.use((request) => {
        if (!!request) {
            log(`Requesting ${request.method} : ${Object(_util_RedactorUtil__WEBPACK_IMPORTED_MODULE_3__["redactNricfinInString"])(request.url)}`);
        }
        return request;
    }, (error) => {
...
const addResponseLogs = (instance) => {
    instance.interceptors.response.use((response) => {
        if (!!response) {
            log(`Responded ${response.config.method}  : ${Object(_util_RedactorUtil__WEBPACK_IMPORTED_MODULE_3__["redactNricfinInString"])(response.config.url)}`);
        }
        return response;
    }, (error) => {
        if (!!error.response) {
            log(`Error occurred while responding to request ${error.response.config.method} :
			${Object(_util_RedactorUtil__WEBPACK_IMPORTED_MODULE_3__["redactNricfinInString"])(error.response.config.url)}`, error);
        }
        else {
