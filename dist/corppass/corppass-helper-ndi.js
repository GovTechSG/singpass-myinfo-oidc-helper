...
class NdiOidcHelper {
    constructor(props) {
        this.constructAuthorizationUrl = (state, nonce) => __awaiter(this, void 0, void 0, function* () {
            const { data: { authorization_endpoint }, } = yield this.axiosClient.get(this.oidcConfigUrl, { headers: this.additionalHeaders });
        ...
        this.getTokens = (authCode) => __awaiter(this, void 0, void 0, function* () {
            const { data: { token_endpoint, issuer }, } = yield this.axiosClient.get(this.oidcConfigUrl, { headers: this.additionalHeaders });
            const params = {
            ...
            const config = {
                headers: Object.assign(Object.assign({}, this.additionalHeaders), { "content-type": "application/x-www-form-urlencoded" }),
            };
            const finalTokenEndpoint = this.proxyBaseUrl ? token_endpoint.replace(issuer, this.proxyBaseUrl) : token_endpoint;
            const response = yield this.axiosClient.post(finalTokenEndpoint, body, config);
    ...
    getAccessTokenPayload(tokens) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { data: { jwks_uri, issuer }, } = yield this.axiosClient.get(this.oidcConfigUrl, { headers: this.additionalHeaders });
                const finalJwksUri = this.proxyBaseUrl ? jwks_uri.replace(issuer, this.proxyBaseUrl) : jwks_uri;
                const { data: { keys }, } = yield this.axiosClient.get(finalJwksUri, { headers: this.additionalHeaders });
