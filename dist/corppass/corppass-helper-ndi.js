...
class NdiOidcHelper {
    ...
    async getAccessTokenPayload(tokens) {
        try {
            ...
            for (const key of keys) {
                try {
                    const verified = await util_1.JweUtil.verifyJWS(access_token, JSON.stringify(key), "json");
            ...
            if (error) {
                ...
                throw new SingpassMyinfoError_1.SingpassMyInfoError('could not verify with any key');
            }
        }
        catch (e) {
    ...
    async getIdTokenPayload(tokens, overrideDecryptKey) {
        try {
            ...
            for (const key of keys) {
                try {
                    const verified = await util_1.JweUtil.verifyJWS(jwsPayload, JSON.stringify(key), "json");
                    return JSON.parse(verified.payload.toString());
                }
                catch (err) {
                    error = err;
     ...
     */
    async getAuthorisationInfoTokenPayload(tokens) {
        try {
            const { data: { "authorization-info_endpoint": authorisationInfoEndpoint, jwks_uri, issuer }, } = await this.axiosClient.get(this.oidcConfigUrl, { headers: this.additionalHeaders });
            ...
            const finalJwksUri = this.proxyBaseUrl ? jwks_uri.replace(issuer, this.proxyBaseUrl) : jwks_uri;
            const { data: { keys }, } = await this.axiosClient.get(finalJwksUri, { headers: this.additionalHeaders });
            const verifiedJws = await util_1.JweUtil.verifyJwsUsingKeyStore(authorisationInfoJws, keys);
            ...
            if (authorisationInfoTokenPayload.TPAuthInfo) {
                authorisationInfoTokenPayload.TPAuthInfo = JSON.parse(authorisationInfoTokenPayload.TPAuthInfo);
            }
            return authorisationInfoTokenPayload;
        }
        catch (e) {
            Logger_1.logger.error("Failed to get authorisation info", e);
            throw e;
        }
    }
    extractActiveAuthResultFromAuthInfoToken(authInfoTokenPayload) {
        ...
        results.forEach((result) => {
            const { Auth_Result_Set: { Row: resultRows }, CPESrvcID: serviceId, } = result;
            const filteredAuthResultSet = resultRows.filter((resultRow) => {
                const { StartDate, EndDate } = resultRow;
                return DateUtils_1.DateUtils.isWithinPeriod(StartDate, EndDate);
            });
