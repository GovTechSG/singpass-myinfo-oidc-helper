...
(function (DateUtils) {
    ...
    function isoToLocalDateTime(iso) {
        ...
        try {
            ...
            const filteredIso = (_a = RegExp(filterRegex).exec(iso)) === null || _a === void 0 ? void 0 : _a[0];
    ...
    function endOfYear(input, zoneId = defaultTz) {
        ...
        if (!zdt)
            return null;
        return zdt
            .with(core_1.TemporalAdjusters.lastDayOfYear())
            .withHour(23)
            .withMinute(59)
            .withSecond(59)
            .withNano(999999999);
     ...
     */
    function isWithinPeriod(startOfPeriod, endOfPeriod, input = core_1.LocalDate.now()) {
        const CORPPASS_DATE_FORMAT = "uuuu-MM-dd";
        const formatter = new core_1.DateTimeFormatterBuilder()
            .appendPattern(CORPPASS_DATE_FORMAT)
            .toFormatter(core_1.ResolverStyle.STRICT);
        const start = core_1.LocalDate.parse(startOfPeriod, formatter);
        const end = core_1.LocalDate.parse(endOfPeriod, formatter);
        return !(input.compareTo(start) < 0 || input.compareTo(end) > 0);
    }
    DateUtils.isWithinPeriod = isWithinPeriod;
